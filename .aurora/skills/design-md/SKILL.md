...
- design-md, design system, tokens visuais, paleta de cores
- tipografia, design.md, semântica visual, consistência de design
- stitch design system, documentação de design, visual descriptions

### Context Patterns
- usuário deseja sistematizar a identidade visual de um projeto Stitch
- necessidade de extrair e documentar tokens de uma tela existente para replicação
- criação de um arquivo DESIGN.md para servir como "fonte da verdade" em gerações futuras
- documentação detalhada de atmosfera (vibe), cores, tipografia e geometria (shapes)
- busca por consistência visual entre múltiplas telas de um mesmo projeto
- tradução de classes técnicas (Tailwind/CSS) para descrições semânticas amigáveis à IA

### Negations (não disparar quando)
- a intenção é apenas refinar o texto de um prompt (Refinar Cognição)
- o foco é a implementação técnica final em React/Next.js (Site Builder)
- trata-se de diagnóstico estratégico de conversão ou UX (CEO-Web)

### Agent Affinity
- Compatível com: stitch-design (provê o contexto de design), taste-design (ajuda na calibração de estilo)
- Informa: stitch-design (entrega o arquivo de referência de tokens)
- Requerido por: nada (atua como o arquivista do sistema visual)

### Observability
- Custo estimado por invocação: BAIXO
- Token budget recomendado: 500
- Latência típica: RÁPIDA 3-5s
