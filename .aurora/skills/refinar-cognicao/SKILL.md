...
- refinar-cognicao, prompt engineering, otimização de prompt, stitch optimization
- clareza semântica, arquitetura de prompt, refinamento de ideia, uix keywords
- prompt para design, especificidade visual, estrutura de página, design system injection

### Context Patterns
- usuário apresenta uma ideia vaga ou rascunho de interface e necessita de um prompt estruturado
- necessidade de aumentar a precisão de ferramentas de geração visual (Stitch)
- injeção de terminologia técnica (GSAP, R3F, Flexbox, Grid) para guiar a IA de design
- transformação de objetivos de negócio em requisitos funcionais e visuais claros
- estruturação de hierarquia de página (Hero, Body, Footer) antes da geração
- refinamento de prompts que geraram resultados insatisfatórios anteriormente

### Negations (não disparar quando)
- a intenção é escrever o código final da aplicação (Site Builder)
- o foco é a criação de um novo sistema de tokens visuais / DESIGN.md (design-md)
- trata-se de auditoria de qualidade ou detecção de bugs (QA Review)

### Agent Affinity
- Compatível com: stitch-design (prepara o terreno para a geração), ceo-design, stitch-loop
- Informa: stitch-design (entrega o prompt refinado), stitch-loop (gera o input para o loop)
- Requerido por: nada (atua como o tradutor entre ideia e comando)

### Observability
- Custo estimado por invocação: BAIXO
- Token budget recomendado: 600
- Latência típica: RÁPIDA 5-8s
