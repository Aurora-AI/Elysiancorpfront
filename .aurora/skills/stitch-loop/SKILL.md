...
- stitch-loop, loop de feedback, refinamento de design
- iteration loop, autonomous builder, baton system
- design review, incremental build, site automation

### Context Patterns
- usuário fornece feedback ou solicita ajustes em uma tela previamente gerada pelo Stitch
- necessidade de iniciar ou manter um ciclo autônomo de construção de site (SITE.md)
- orquestração de múltiplas telas em um fluxo de "passagem de bastão" (next-prompt.md)
- atualização progressiva de sitemaps, roadmaps e documentação de projeto (metadata.json)
- refinamento incremental de mockups baseado em critérios de "QA" ou feedback humano
- integração técnica de novas páginas geradas no diretório public/ do site

### Negations (não disparar quando)
- a intenção é a criação isolada de uma nova tela sem contexto de projeto (Stitch Design)
- o foco é a extração inicial de tokens ou criação de biblioteca (Design-MD)
- trata-se apenas de curadoria estética ou definição de "vibe" (Taste Design)

### Agent Affinity
- Compatível com: stitch-design (executa a geração), design-md (consome contexto de sistema), taste-design (valida qualidade visual)
- Requer: stitch-design (para chamadas MCP), design-md (para referencial de tokens)
- Requerido por: nada (atua como o orquestrador de ciclo de vida do projeto)

### Observability
- Custo estimado por invocação: MÉDIO (gerenciamento de estado + chamadas Stitch)
- Token budget recomendado: 1000
- Latência típica: MÉDIA 12-18s (envolve leitura de múltiplos arquivos de contexto)
