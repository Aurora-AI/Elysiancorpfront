...
- shadcn-ui, shadcn, componentes ui, radix ui
- tailwind components, npx shadcn, componentes reativos
- library de componentes, shadcn cli, reusable components

### Context Patterns
- usuário solicita a adição ou integração de componentes shadcn/ui ao projeto
- necessidade de instalar novos elementos de interface via CLI (npx shadcn add)
- busca por documentação técnica ou exemplos de uso de componentes (Button, Dialog, etc.)
- customização de temas e tokens vinculados ao ecossistema shadcn (css variables, baseColor)
- integração de bibliotecas de acessibilidade (Radix UI) com Tailwind CSS
- construção de blocos de UI complexos (dashboards, forms) baseados na estrutura shadcn

### Negations (não disparar quando)
- a intenção é geração visual de mockups ou imagens de alta fidelidade (Stitch)
- o foco é busca e curadoria de referências estéticas premium externas (Agente Awwwards)
- trata-se de redação de copy, narrativa de venda ou dor latente (CEO-Copy)

### Agent Affinity
- Compatível com: site-builder (fornece a base técnica para construção), ceo-design (alinhamento visual)
- Informa: site-builder (requisitos de instalação e props de componentes)
- Requerido por: nada (atua como o especialista em blocos de construção UI)
