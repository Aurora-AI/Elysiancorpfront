  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      layersRef.current.forEach((layer, index) => {
        if (!layer) return;
        const depth = (index + 1) * 30; // Increased depth for more impact
        const moveX = x * (index + 1) * 0.4;
        const moveY = y * (index + 1) * 0.4;
  ...
  return (
    <div className="halide-root">
      ...
      <div className="interface-grid">
        <div style={{ fontWeight: 700, fontSize: '0.8rem', letterSpacing: '0.2em' }}>HALIDE_CORE</div>
        <div style={{ textAlign: 'right', fontFamily: 'monospace', color: 'var(--accent)', fontSize: '0.7rem', letterSpacing: '0.1em' }}>
